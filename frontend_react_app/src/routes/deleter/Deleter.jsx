...
function Deleter() {
    ...
    const [isDeleterVisible, setIsDeleterVisible] = useState(true);
    const [accessToken, setAccessToken] = useState(localStorage.getItem('accessToken') || '');
    const refreshToken = localStorage.getItem('refreshToken');
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.get(`http://localhost:3000/postDeleter?postId=${postId}&postType=${postType}`, {
                method: 'GET',
                credentials: 'include',
                headers: {
            ...
            if (response.data.post) {
                ...
                const postToBeDeleted = response.data.post;
                try {
                    switch (tableName) {
                        ...
                        case 'art_post':
                        case 'politics_post':
                            setPostElement(<Post_template postToBeDeleted={postToBeDeleted} />);
                            break;
                        case 'philosophy_article_post':
                        case 'science_article_post':
                            setPostElement(<Article_template postToBeDeleted={postToBeDeleted} />);
                            break;
                        case 'daily_quote':
                        case 'politics_hero_post':
                            setPostElement(<Small_post_template postToBeDeleted={postToBeDeleted} />);
                            break;
                        case 'finance_slide_post':
                            setPostElement(<Slide_template postToBeDeleted={postToBeDeleted} />);
                            break;
                        case 'technology_box_post':
                            setPostElement(<Box_post_template postToBeDeleted={postToBeDeleted} />);
                            break;
                        case 'technology_body_post':
                            setPostElement(<Body_post_template postToBeDeleted={postToBeDeleted} />);
                            break;
                        case 'art_body_post':
                            setPostElement(<Body_post2_template postToBeDeleted={postToBeDeleted} />);
                            break;
                        case 'politics_body_post':
                            setPostElement(<Body_post3_template postToBeDeleted={postToBeDeleted} />);
                            break;
                        default:
                            break;
                    }
        ...
        <>
            {isDeleterVisible && (
                <div className="template_container">
                    <h3>Enter ID Number to Delete</h3>
